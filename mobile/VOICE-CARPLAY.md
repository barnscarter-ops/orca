...
flowchart TD
    ...
    Entitlement{"Apple managed voice-conversation entitlement?"} -- Approved --> Enable["Set enableCarPlayScene to true"]
    Enable --> CarPlayScene["Generate CPTemplateApplicationScene"]
    CarPlayScene --> VoiceTemplate["CPVoiceControlTemplate ready state"]
    VoiceTemplate --> Desktop
    Entitlement -- Pending or denied --> Intent
```
...
- The config plugin contains a compiler-verified `OrcaCarPlaySceneDelegate` and voice-control
  template scaffold, but `enableCarPlayScene` remains `false`. After Apple grants the managed
  capability, set it to `true` and add `com.apple.developer.carplay-voice-based-conversation` to the
  app entitlement before generating the release build.
